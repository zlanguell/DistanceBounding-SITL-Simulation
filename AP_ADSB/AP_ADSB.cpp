...
#include <AP_DistanceBound/AP_DistanceBound.h>
...
void AP_ADSB::update(void)
{
    ...
    // check current list for vehicles that time out or failed a distance bounding attempt
    uint16_t index = 0;
    while (index < in_state.vehicle_count) {
        // check list and drop stale vehicles. When disabled, the list will get flushed
        if ( (now - in_state.vehicle_list[index].last_update_ms > VEHICLE_TIMEOUT_MS) /*|| (in_state.vehicle_list[index].info.validated == false)*/ ){
...
void AP_ADSB::handle_vehicle(const mavlink_message_t* packet)
{
    ...
    if (vehicle_loc.is_zero() ||
        ...
        std::thread(&AP_ADSB::setupDistanceBounding, this, vehicle).detach();
        //AP_ADSB::setupDistanceBounding(vehicle);
...
void AP_ADSB::setupDistanceBounding(AP_ADSB::adsb_vehicle_t vehicle_db){
  AP_DistanceBound* dbThread = new AP_DistanceBound(vehicle_db);
  dbThread->setup(vehicle_db);
  delete dbThread;
}
